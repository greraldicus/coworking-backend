...
from app.schemas import PersonWithTenureSchema, PersonCreateSchema, JwtPayloadSchema
from app.auth import get_token_decoded
...
async def get_person_endpoint(
        ...
        token: dict = Depends(get_token_decoded)
):
    role = await get_role_model_by_user_id(db=db, user_id=token.get('sub'))
